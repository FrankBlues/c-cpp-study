...
int main()
{
  ...
  std::cout << "1-norm(n)     = " << n.cwiseAbs().colwise().sum().maxCoeff()
            << " == "             << n.colwise().lpNorm<1>().maxCoeff() << std::endl;
 
  std::cout << "infty-norm(n) = " << n.cwiseAbs().rowwise().sum().maxCoeff()
            << " == "             << n.rowwise().lpNorm<1>().maxCoeff() << std::endl;
