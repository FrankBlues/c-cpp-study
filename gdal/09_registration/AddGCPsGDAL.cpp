...
AddGCPsGDAL::AddGCPsGDAL(std::string inFileName, std::string gcpFilePath, std::string outFileName, std::string gdalFormat, GDALDataType gdalDataType)
{
	std::string gcpImage = "";
	GDALAllRegister();
...
void AddGCPsGDAL::readGCPFile(std::string gcpFilePath, std::vector<GCPImg2MapNode*> *gcps)
{
	TextUtils textUtils;
	...
	if(gcpFile.is_open())
	{
		...
		while(!gcpFile.eof())
		{
			...
			if((!textUtils.lineStart(strLine, '#')) & (!textUtils.blankline(strLine)))
			{
				...
				gcp = new GCPImg2MapNode(mathUtils.strtodouble(tokens->at(0)),
...
void AddGCPsGDAL::convert2GDALGCP(std::vector<GCPImg2MapNode*> *gcps, GDAL_GCP *gdalGCPList)
{
	
	unsigned int gcpNum=0;
	std::string gcpInfo = "";
	
	std::vector<GCPImg2MapNode*>::iterator iterGCPs;
...
void AddGCPsGDAL::copyImageWithoutSpatialRef(std::string inFileName, std::string outFileName, std::string gdalFormat, GDALDataType gdalDataType)
{
	GDALAllRegister();
	ImageUtils imgUtils;
	...
	try
	{
		...
		if(gdalDriver == NULL)
		{
			throw ImageException("Requested GDAL driver does not exists..");
		}
		...
		{
			throw ImageException("Output image could not be created. Check filepath.");
		}
