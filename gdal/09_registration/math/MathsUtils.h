...
struct DllExport HistBin2D
{
    HistBin2D(double binCentre1, double binCentre2, double freq)
    {
...
struct DllExport LinearFitVals
{
    double  slope;
    double  intercept;
    double  coeff;
...
class DllExport MathsUtils
    ...
    public:
        ...
        void generateStats(std::vector<double> *data, StatsSummary *stats);
        void initStatsSummary(StatsSummary *stats);
        void initStatsSummaryValues(StatsSummary *stats);
        ...
        std::vector<std::vector<HistBin2D>* >* calc2DHistogram(std::vector<double> *data1, double minVal1, double maxVal1, double binWidth1, std::vector<double> *data2, double minVal2, double maxVal2, double binWidth2, bool norm);
        ...
        LinearFitVals* performLinearFit(double *xData, double *yData, size_t nVals, double noDataVal);
        void performLinearFit(double *xData, double *yData, size_t nVals, double noDataVal, LinearFitVals *fitVals);
        double predFromLinearFit(double val, LinearFitVals *fitVals, double minAccVal, double maxAccVal);
    };


class DllExport CoordsTransform
{
public:
    CoordsTransform(){};
