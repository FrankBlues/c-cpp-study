...
Vector* Vectors::createVector(int n)
{
	if(n < 1)
	{
		throw VectorsException("Size n must be at least 1.");
	}
...
void Vectors::freeVector(Vector *vector)
{
	if(vector != nullptr)
	{
...
void Vectors::printVector(Vector *vector)
{		
	int index = 0;
...
void Vectors::printGSLVector(gsl_vector *vector)
{
...
void Vectors::saveVector2GridTxt(Vector *vector, std::string filepath)
{
...
void Vectors::saveVector2txt(Vector *vector, std::string filepath)
{
...
void Vectors::saveGSLVector2GridTxt(gsl_vector *gslVector, std::string filepath)
{
	Vector *Vector;
	Vector = this->convertGSL2Vector(gslVector);
	this->saveVector2GridTxt(Vector, filepath);
	this->freeVector(Vector);
}

void Vectors::saveGSLVector2Txt(gsl_vector *gslVector, std::string filepath)
{
	Vector *Vector;
	Vector = this->convertGSL2Vector(gslVector);
	this->saveVector2txt(Vector, filepath);
	this->freeVector(Vector);
}	

Vector* Vectors::readVectorFromTxt(std::string filepath)
{
	Vector *vector = new Vector();
	...
	else
	{
			...
			if(strLine.length() > 0)
			{
				...
				else if(lineCounter == 1)
				{
					...
					for(int i = 0; i < lineLength; i++)
					{
						...
						{
							throw VectorsException("Too many data values, compared to header.");
						}
					}
					...
					{
						throw VectorsException("An incorrect number of data points were read in.");
					}
		...
		if(lineCounter < 2)
		{
			throw VectorsException("A complete vector has not been reconstructed.");
		}
		inputVector.close();
	}
	return vector;
...
Vector* Vectors::readVectorFromGridTxt(std::string filepath)
{
	Vector *vector = new Vector();
	std::ifstream inputVector;
	inputVector.open(filepath.c_str());
...
gsl_vector* Vectors::readGSLVectorFromTxt(std::string filepath)
{
	Vector *Vector;
	gsl_vector *gslVector;
	Vector = this->readVectorFromTxt(filepath);
	gslVector = this->convert2GSLVector(Vector);
	this->freeVector(Vector);
	return gslVector;
}

gsl_vector* Vectors::readGSLVectorFromGridTxt(std::string filepath)
{
	Vector *Vector;
	gsl_vector *gslVector;
	Vector = this->readVectorFromGridTxt(filepath);
	gslVector = this->convert2GSLVector(Vector);
	this->freeVector(Vector);
	return gslVector;
}

gsl_vector* Vectors::convert2GSLVector(Vector *inVector)
{
...
Vector* Vectors::convertGSL2Vector(gsl_vector *inVector)
{
	/// Converts GSL vector to a  vector
...
double Vectors::dotProductVectorVectorGSL(gsl_vector *inVectorA, gsl_vector *inVectorB)
{
	...
	*/
	...
	{
		throw VectorsException("Vectors are not of equal length");
	}
...
gsl_vector* Vectors::crossProductVectorVectorGSL(gsl_vector *inVectorA, gsl_vector *inVectorB)
{
...
double Vectors::euclideanDistance(Vector *vecA, Vector *vecB)
{
	if(vecA->n != vecB->n)
	{
		throw VectorsException("Vectors need to be of the same length for the distance to be calculated.");
	}
