...
double MathsUtils::strtodouble(std::string inValue)
{
    double outValue = 0;
    try
    {
...
float MathsUtils::strtofloat(std::string inValue)
{
    float outValue = 0;
    try
    {
...
int MathsUtils::strtoint(std::string inValue)
{
    int outValue = 0;
    try
    {
...
unsigned int MathsUtils::strtounsignedint(std::string inValue)
{
...
long MathsUtils::strtolong(std::string inValue)
{
    long outValue = 0;
    try
    {
...
unsigned long MathsUtils::strtounsignedlong(std::string inValue)
{
...
std::string MathsUtils::floattostring(float number)
{
    std::string outValue = "";
    try
    {
...
std::string MathsUtils::doubletostring(double number)
{
    std::string outValue = "";
    try
    {
...
std::string MathsUtils::inttostring(int number)
{
    std::string outValue = "";
    try
    {
...
std::string MathsUtils::uinttostring(unsigned int number)
{
    std::string outValue = "";
    try
    {
...
std::string MathsUtils::longtostring(long number)
{
    std::string outValue = "";
    try
    {
...
float MathsUtils::degreesToRadians(float angle)
{
    return angle * (M_PI/180);
}

float MathsUtils::radiansToDegrees(float angle)
{
    return angle * (180 / M_PI);
}

void MathsUtils::getPowerSet(int numItems, int *numBands, int **indexes, int numSets)
{
    if(numItems == 0)
    {
        throw MathException("A power set cannot be created from zero items");
    }
    else if(numItems == 1)
    {
        if(numSets != 1)
        {
            throw MathException("numSets not correct.");
        }
        numBands[0] = 1;
        indexes[0] = new int[1];
        indexes[0][0] = 0;
    ...
    {
        throw MathException("Power sets are currently hardcoded and only support 1 - 5 items.");
    }
    
}

int MathsUtils::roundUp(double number)
{
    return static_cast<int>(number+0.5);
}

Vector* MathsUtils::calculateCumulativeArea(Vector *inVec, Matrix *widths)
{
    Vectors vecUtils;
...
Matrix* MathsUtils::calculateCumulativeArea(Matrix *inData, Matrix *widths)
{
...
void MathsUtils::fitPlane(double *x, double *y, double *z, unsigned int numValues, double *a, double *b, double *c)
{
    Matrices matrices;
    Matrix *matrixA = NULL;
    Matrix *matrixB = NULL;
...
void MathsUtils::generateStats(std::vector<double> *data, StatsSummary *stats)
{
    try
    {
        if(data->size() > 0)
...
void MathsUtils::initStatsSummary(StatsSummary *stats)
{
    if(stats != NULL)
    {
        stats->calcMin = false;
...
void MathsUtils::initStatsSummaryValues(StatsSummary *stats)
{
    if(stats != NULL)
    {
        stats->min = 0;
...
bool MathsUtils::angleWithinRange(float angle, float lower, float upper)
{
...
double MathsUtils::calcPercentile(float percentile, double *binBounds, double binWidth, unsigned int numBins, unsigned int *hist)
{
    ...
    try
    {
        ...
        else
        {
            ...
            if(!foundBin)
            {
                throw MathException("Could not find percentile bin! Something I cannot explain has gone wrong!");
            }
...
double MathsUtils::calcPercentile(float percentile, double histMinVal, double binWidth, unsigned int numBins, unsigned int *hist)
{
    double percentVal = 0.0;
    try
    {
...
double* MathsUtils::calcMeanVector(double **data, size_t n, size_t m, size_t sMIdx, size_t eMIdx)
{
...
double** MathsUtils::calcCovarianceMatrix(double **data, double *meanVec, size_t n, size_t m, size_t sMIdx, size_t eMIdx)
{
    size_t numVals = eMIdx - sMIdx;
...
std::vector<std::pair<size_t, double> >* MathsUtils::sampleUseHistogramMethod(std::vector<std::pair<size_t, double> > *inData, double minVal, double maxVal, double binWidth, float propOfPop)
{
...
std::vector<std::pair<size_t, double> >** MathsUtils::calcHistogram(std::vector<std::pair<size_t, double> > *inData, double minVal, double maxVal, double binWidth, size_t *numBins)
{
...
std::vector<std::pair<double, double> >* MathsUtils::calcHistogram(std::vector<double> *data, double minVal, double maxVal, double binWidth, bool norm)
{
...
std::vector<std::vector<HistBin2D>* >* MathsUtils::calc2DHistogram(std::vector<double> *data1, double minVal1, double maxVal1, double binWidth1, std::vector<double> *data2, double minVal2, double maxVal2, double binWidth2, bool norm)
{
    std::vector<std::vector<HistBin2D>* > *hist = new std::vector<std::vector<HistBin2D>* >();
    try
    {
        if(data1->size() != data2->size())
        {
            throw MathException("The two input datasets are not the same size.");
        }
        ...
        {
            std::vector<HistBin2D> *tmpHist = new std::vector<HistBin2D>();
            tmpHist->reserve(numBins2);
            
            binCentre2 = minVal2 + (binWidth2/2);
            for(size_t j = 0; j < numBins2; ++j)
            {
                tmpHist->push_back(HistBin2D(binCentre1, binCentre2, 0.0));
...
unsigned int* MathsUtils::calcHistogram(double *data, size_t numVals, double binWidth, double *minVal, double *maxVal, unsigned int *numBins, bool ignoreFirstVal)
{
    ...
    try
    {
        ...
        *maxVal = 0.0;
        ...
        if(((*maxVal) - (*minVal)) < (binWidth*2))
        {
            ...
            throw MathException("Bin width is too large or not enough variation within the data to create a histogram.");
        }
...
LinearFitVals* MathsUtils::performLinearFit(double *xData, double *yData, size_t nVals, double noDataVal)
{
    LinearFitVals *fitVals = new LinearFitVals();
    try
    {
        double sumy = 0;
        double sumx = 0;
...
void MathsUtils::performLinearFit(double *xData, double *yData, size_t nVals, double noDataVal, LinearFitVals *fitVals)
{
    try
    {
        double sumy = 0;
...
double MathsUtils::predFromLinearFit(double val, LinearFitVals *fitVals, double minAccVal, double maxAccVal)
{
    double outVal = 0.0;
...
void CoordsTransform::transformPoint(OGRSpatialReference *input_spat_ref, OGRSpatialReference *output_spat_ref, double inX, double inY, double *outX, double *outY)
{
    ...
    {
        throw ::BasicException("Coordinate System Transformation failed.");
    }
...
void CoordsTransform::transformBBOX(OGRSpatialReference *input_spat_ref, OGRSpatialReference *output_spat_ref, OGREnvelope *inBBOX, OGREnvelope *outBBOX)
{
