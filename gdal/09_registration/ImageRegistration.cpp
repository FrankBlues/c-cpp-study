...
ImageRegistration::ImageRegistration(GDALDataset *reference, GDALDataset *floating): referenceIMG(NULL), floatingIMG(NULL), overlap(NULL), overlapDefined(false)
{
	this->referenceIMG = reference;
	this->floatingIMG = floating;
}

void ImageRegistration::runCompleteRegistration()
{
...
void ImageRegistration::findOverlap()
{
	try 
	{
...
void ImageRegistration::defineFirstTiePoint(unsigned int *startXOff, unsigned int *startYOff, unsigned int numXPts, unsigned int numYPts, unsigned int gap)
{
	if(!overlapDefined)
	{
		throw RegistrationException("The overlap needs to be defined before first tie point can be defined.");
	}
...
float ImageRegistration::findTiePointLocation(TiePoint *tiePt, unsigned int windowSize, unsigned int searchArea, ImageSimilarityMetric *metric, float metricThreshold, unsigned int subPixelResolution, float *moveInX, float *moveInY)
{
			...
			for(int xShift = xShiftStart; xShift <= xShiftEnd; ++xShift)
			{
				try
				{
					...
					if((overlapWidth > 0) & (overlapHeight > 0))
					{
						...
						{
							throw RegistrationException("The number of data values read from the images does not match.");
						}
...
float ImageRegistration::findTiePointLocation(TiePoint *tiePt, unsigned int windowSize, unsigned int searchArea, ImageSimilarityMetric *metric, unsigned int subPixelResolution, float *moveInX, float *moveInY)
{
	float distanceMoved = 0;
...
float ImageRegistration::findExtreme(bool findMin, gsl_vector *coefficients, unsigned int order, float minRange, float maxRange, unsigned int resolution, float *extremeVal)
{
...
void ImageRegistration::getImageOverlapFloat(GDALDataset **datasets, int numDS,  float **dsOffsets, int *width, int *height, double *gdalTransform)
{
	/**
	...
	try
	{
		for(int i = 0; i < numDS; i++)
		{
			...
			{
				throw RegistrationException("No projection transformation has been provided..");
			}
			...
			else
			{
				...
				{
					throw RegistrationException("Not all image bands have the same resolution..");
				}
				
				if(transformations[i][2] != rotateX & transformations[i][4] != rotateY)
				{
					throw RegistrationException("Not all image bands have the same rotation..");
				}
		...
		if(maxX - minX <= 0)
		{
			throw RegistrationException("Images do not overlap in the X axis");
		}
		
		if(maxY - minY <= 0)
		{
			throw RegistrationException("Images do not overlap in the Y axis");
		}
...
void ImageRegistration::getImageOverlapWithFloatShift(float xShift, float yShift, int **dsOffsets, int *width, int *height, double *gdalTransform, OGREnvelope *env, float *remainderX, float *remainderY)
{
	if(!overlapDefined)
	{
		throw RegistrationException("The overlap needs to be defined.");
	}
	...
	try 
	{
		...
		{
			throw RegistrationException("The overlapping region of the images does not intersect with the envelop provided");
		}
...
double ImageRegistration::calcStdDev(float **data, unsigned int numVals, unsigned int numDims)
{
...
void ImageRegistration::exportTiePointsENVIImage2MapImpl(std::string filepath, std::list<TiePoint*> *tiePts)
{
...
void ImageRegistration::exportTiePointsENVIImage2ImageImpl(std::string filepath, std::list<TiePoint*> *tiePts)
{
	std::ofstream outPtsFile(filepath.c_str(), std::ios::out | std::ios::trunc);
...
void ImageRegistration::exportTiePointsImage2MapImpl(std::string filepath, std::list<TiePoint*> *tiePts)
{
	std::ofstream outPtsFile(filepath.c_str(), std::ios::out | std::ios::trunc);
	...
	outPtsFile << "# Lib Image to Map GCP File\n";
...
void ImageRegistration::exportTiePointsMapOffsImpl(std::string filepath, std::list<TiePoint*> *tiePts)
{
	std::ofstream outPtsFile(filepath.c_str(), std::ios::out | std::ios::trunc);
	...
	outPtsFile << "# Lib Map offsets GCP File\n";
